...
export const users = pgTable(
  ...
  table => [
    ...
    unique('users_provider_provider_id_unique').on(
      table.provider,
      table.providerId
    ),
  ]
...
export const orders = pgTable(
  ...
  table => [
    ...
    check(
      ...
      sql`(payment_provider <> 'none'::text) OR (payment_intent_id IS NULL)`
    ),
    check(
      'orders_payment_provider_valid',
      sql`payment_provider = ANY (ARRAY['stripe'::text, 'none'::text])`
    ),
    check(
      'orders_total_amount_minor_non_negative',
      sql`total_amount_minor >= 0`
    ),
  ]
);
...
export const products = pgTable(
  ...
  table => [
    uniqueIndex('products_slug_unique').using(
      'btree',
...
export const productPrices = pgTable(
  ...
  table => [
    index('product_prices_currency_idx').using(
      'btree',
      table.currency.asc().nullsLast().op('enum_ops')
    ),
    uniqueIndex('product_prices_product_currency_uq').using(
      'btree',
      table.productId.asc().nullsLast().op('uuid_ops'),
      table.currency.asc().nullsLast().op('uuid_ops')
    ),
    foreignKey({
    ...
    check(
      ...
      sql`(original_price_minor IS NULL) OR (original_price_minor > price_minor)`
    ),
...
export const orderItems = pgTable(
  ...
  table => [
    ...
    check(
      ...
      sql`line_total_minor = (unit_price_minor * quantity)`
    ),
    check(
      'order_items_line_total_minor_non_negative',
      sql`line_total_minor >= 0`
    ),
...
export const quizAttempts = pgTable(
  ...
  table => [
    ...
    index('quiz_attempts_quiz_percentage_completed_at_idx').using(
      ...
      table.percentage.asc().nullsLast().op('timestamptz_ops'),
      table.completedAt.asc().nullsLast().op('numeric_ops')
    ),
...
export const stripeEvents = pgTable(
  ...
  table => [
    uniqueIndex('stripe_events_event_id_idx').using(
      'btree',
...
export const questionTranslations = pgTable(
  ...
  table => [
    ...
    primaryKey({
      columns: [table.questionId, table.locale],
