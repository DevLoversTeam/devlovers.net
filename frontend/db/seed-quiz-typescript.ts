...
const CATEGORY_SLUG = 'typescript';
...
const QUIZ_METADATA = {
  ...
  translations: {
    uk: {
      ...
      description: 'Перевірте свої знання базових концепцій TypeScript: типи даних, інтерфейси, generics, класи, модулі та утилітні типи.',
    },
    en: {
      title: 'TypeScript Fundamentals',
      description: 'Test your knowledge of TypeScript basics: data types, interfaces, generics, classes, modules and utility types.',
    },
    pl: {
      title: 'Podstawy TypeScript',
      description: 'Sprawdź swoją wiedzę o podstawach TypeScript: typy danych, interfejsy, generics, klasy, moduły i typy narzędziowe.',
    },
  },
};
...
async function loadQuestions(partNumber: number): Promise<QuestionData[]> {
  const partPath = join(process.cwd(), 'parse', 'typescript', 'fundamentals', `typescript-quiz-part${partNumber}.json`);
...
async function seedQuizFromJson() {
  ...
  if (!partArg) {
    ...
    console.log('Usage: npx tsx db/seeds/seed-quiz-typescript.ts <part-number>');
    console.log('Example: npx tsx db/seeds/seed-quiz-typescript.ts 1');
    console.log('Or upload all: npx tsx db/seeds/seed-quiz-typescript.ts all');
    process.exit(1);
  }

  console.log('Starting TypeScript quiz seed...\n');
