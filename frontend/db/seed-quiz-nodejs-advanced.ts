...
const QUIZ_METADATA = {
  ...
  translations: {
    uk: {
      ...
      description: 'Поглиблені знання Node.js: мікросервіси, gRPC, GraphQL, NestJS, serverless, CI/CD, Kubernetes, безпека та масштабування.',
    },
    en: {
      title: 'Node.js Advanced',
      description: 'Advanced Node.js knowledge: microservices, gRPC, GraphQL, NestJS, serverless, CI/CD, Kubernetes, security, and scaling.',
    },
    pl: {
      title: 'Node.js Advanced',
      description: 'Zaawansowana wiedza o Node.js: mikroserwisy, gRPC, GraphQL, NestJS, serverless, CI/CD, Kubernetes, bezpieczeństwo i skalowanie.',
    },
  },
};
...
async function loadQuestions(partNumber: number): Promise<QuestionData[]> {
  const partPath = join(projectRoot, 'json', 'quizzes', 'node', 'advanced', `nodejs-advanced-quiz-part${partNumber}.json`);
...
async function seedQuizFromJson() {
  ...
  if (!partArg) {
    ...
    console.log('Usage: npx tsx db/seeds/seed-quiz-nodejs-advanced.ts <part-number>');
    console.log('Example: npx tsx db/seeds/seed-quiz-nodejs-advanced.ts 1');
    console.log('Or upload all: npx tsx db/seeds/seed-quiz-nodejs-advanced.ts all');
    process.exit(1);
  }

  console.log('Starting Node.js Advanced quiz seed...\n');
