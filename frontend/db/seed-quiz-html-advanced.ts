...
const QUIZ_METADATA = {
  ...
  translations: {
    uk: {
      title: 'HTML Поглиблений',
      description: 'Перевірте поглиблені знання HTML: Web Components, Shadow DOM, API браузера, доступність, SEO, продуктивність та сучасні можливості.',
    },
    en: {
      title: 'HTML Advanced',
      description: 'Test your advanced HTML knowledge: Web Components, Shadow DOM, browser APIs, accessibility, SEO, performance and modern features.',
    },
    pl: {
      title: 'HTML Zaawansowany',
      description: 'Sprawdź zaawansowaną wiedzę o HTML: Web Components, Shadow DOM, API przeglądarki, dostępność, SEO, wydajność i nowoczesne funkcje.',
    },
  },
};
...
async function loadQuestions(partNumber: number): Promise<QuestionData[]> {
  const partPath = join(process.cwd(), 'parse', 'html', 'advanced', `html-advanced-quiz-part${partNumber}.json`);
...
async function seedQuizFromJson() {
  ...
  if (!partArg) {
    ...
    console.log('Usage: npx tsx db/seeds/seed-quiz-html-advanced.ts <part-number>');
    console.log('Example: npx tsx db/seeds/seed-quiz-html-advanced.ts 1');
    console.log('Or upload all: npx tsx db/seeds/seed-quiz-html-advanced.ts all');
    process.exit(1);
  }

  console.log('Starting HTML Advanced quiz seed...\n');
