...
const QUIZ_METADATA = {
  ...
  translations: {
    uk: {
      ...
      description:
        'Перевірте свої знання базових концепцій Node.js: event loop, модулі, асинхронність, Express.js, тестування та деплой.',
    },
    en: {
      title: 'Node.js Fundamentals',
      description:
        'Test your knowledge of Node.js basics: event loop, modules, asynchronicity, Express.js, testing, and deployment.',
    },
    pl: {
      title: 'Podstawy Node.js',
      description:
        'Sprawdź swoją wiedzę o podstawach Node.js: event loop, moduły, asynchroniczność, Express.js, testowanie i deployment.',
    },
  },
};
...
async function loadQuestions(partNumber: number): Promise<QuestionData[]> {
  const partPath = join(
    ...
    'node',
    'beginner_medium',
    `nodejs-quiz-part${partNumber}.json`
  );
...
async function seedQuizFromJson() {
  ...
  if (!partArg) {
    ...
    console.log(
      'Usage: npx tsx db/seeds/seed-quiz-nodejs-fundamentals.ts <part-number>'
    );
    console.log('Example: npx tsx db/seeds/seed-quiz-nodejs-fundamentals.ts 1');
    console.log(
      'Or upload all: npx tsx db/seeds/seed-quiz-nodejs-fundamentals.ts all'
    );
    process.exit(1);
  }

  console.log('Starting Node.js Fundamentals quiz seed...\n');
