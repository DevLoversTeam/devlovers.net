...
export function coercePriceFromDb(
  price: unknown,
  context?: { field?: string; productId?: string }
): number {
...
export function calculateOrderTotal(
  items: Array<{ lineTotal: number; lineTotalCents?: MoneyCents }>
): number {
  const normalized = items.map(
    item => item.lineTotalCents ?? toCents(item.lineTotal)
  );
...
export function ensureSufficientStock(items: PricedItemInput[]) {
  for (const item of items) {
    if (item.stock < 0) {
      throw new Error(
        `Invalid stock configuration for product ${item.productId}`
      );
    }
