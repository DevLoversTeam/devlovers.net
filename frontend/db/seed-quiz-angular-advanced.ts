...
const QUIZ_METADATA = {
  ...
  translations: {
    uk: {
      ...
      description:
        'Поглиблені концепції Angular: lifecycle hooks, DI декоратори, NgRx, кастомні директиви, SSR, Signals, OnPush та оптимізація продуктивності.',
    },
    en: {
      title: 'Angular Advanced',
      description:
        'Advanced Angular concepts: lifecycle hooks, DI decorators, NgRx, custom directives, SSR, Signals, OnPush and performance optimization.',
    },
    pl: {
      title: 'Angular Advanced',
      description:
        'Zaawansowane koncepcje Angular: lifecycle hooks, dekoratory DI, NgRx, niestandardowe dyrektywy, SSR, Signals, OnPush i optymalizacja wydajności.',
    },
  },
};
...
async function loadQuestions(partNumber: number): Promise<QuestionData[]> {
  const partPath = join(
    ...
    'angular',
    'advanced',
    `angular-advanced-quiz-part${partNumber}.json`
  );
...
async function seedQuizFromJson() {
  ...
  if (!partArg) {
    ...
    console.log(
      'Usage: npx tsx db/seeds/seed-quiz-angular-advanced.ts <part-number>'
    );
    console.log('Example: npx tsx db/seeds/seed-quiz-angular-advanced.ts 1');
    console.log(
      'Or upload all: npx tsx db/seeds/seed-quiz-angular-advanced.ts all'
    );
    process.exit(1);
  }

  console.log('Starting Angular Advanced quiz seed...\n');
