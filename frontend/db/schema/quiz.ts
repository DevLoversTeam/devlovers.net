 import { pgTable, uuid, varchar, text, integer, boolean, timestamp, decimal, jsonb,
   primaryKey, index, unique } from 'drizzle-orm/pg-core';
  ...
  'cascade' }),
    ...
    quizDisplayOrderIdx:
  index('quiz_questions_quiz_display_order_idx').on(table.quizId,
  table.displayOrder),
  }));

  export const quizQuestionContent = pgTable('quiz_question_content', {
    quizQuestionId: uuid('quiz_question_id').notNull().references(() =>
  quizQuestions.id, { onDelete: 'cascade' }),
    locale: varchar('locale', { length: 5 }).notNull(),
  ...
  quizQuestions.id, { onDelete: 'cascade' }),
    ...
    questionDisplayOrderIdx:
  index('quiz_answers_question_display_order_idx').on(table.quizQuestionId,
  table.displayOrder),
  }));

  export const quizAnswerTranslations = pgTable('quiz_answer_translations', {
    quizAnswerId: uuid('quiz_answer_id').notNull().references(() => quizAnswers.id, {
   onDelete: 'cascade' }),
    locale: varchar('locale', { length: 5 }).notNull(),
  ...
  'cascade' }),
    ...
    userCompletedAtIdx: index('quiz_attempts_user_completed_at_idx').on(table.userId,
   table.completedAt),
    quizPercentageCompletedAtIdx:
  index('quiz_attempts_quiz_percentage_completed_at_idx').on(table.quizId,
  table.percentage, table.completedAt),
    quizIntegrityScoreIdx:
  index('quiz_attempts_quiz_integrity_score_idx').on(table.quizId,
  ...
  export const quizAnswerTranslationsRelations = relations(quizAnswerTranslations, ({
   one }) => ({
