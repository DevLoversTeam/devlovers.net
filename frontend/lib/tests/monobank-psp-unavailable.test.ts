...
const __prevMonoToken = process.env.MONOBANK_ACQUIRING_TOKEN;
const __prevStatusSecret = process.env.SHOP_STATUS_TOKEN_SECRET;

beforeAll(() => {
  process.env.RATE_LIMIT_DISABLED = '1';
  process.env.PAYMENTS_ENABLED = 'true';
  process.env.MONOBANK_ACQUIRING_TOKEN = 'test_mono_token';
...
afterAll(() => {
  ...
  if (__prevPaymentsEnabled === undefined)
    delete process.env.PAYMENTS_ENABLED;
  else process.env.PAYMENTS_ENABLED = __prevPaymentsEnabled;

  if (__prevMonoToken === undefined)
    delete process.env.MONOBANK_ACQUIRING_TOKEN;
  else process.env.MONOBANK_ACQUIRING_TOKEN = __prevMonoToken;
