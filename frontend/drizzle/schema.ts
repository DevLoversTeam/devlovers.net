import { pgTable, unique, serial, text, foreignKey, jsonb, integer, index, uuid, boolean, varchar, timestamp, numeric, primaryKey } from "drizzle-orm/pg-core"
...
export const quizAttempts = pgTable("quiz_attempts", {
	...
	index("quiz_attempts_quiz_integrity_score_idx").using("btree", table.quizId.asc().nullsLast().op("uuid_ops"), table.integrityScore.asc().nullsLast().op("int4_ops")),
	index("quiz_attempts_quiz_percentage_completed_at_idx").using("btree", table.quizId.asc().nullsLast().op("numeric_ops"), table.percentage.asc().nullsLast().op("numeric_ops"), table.completedAt.asc().nullsLast().op("uuid_ops")),
	index("quiz_attempts_user_completed_at_idx").using("btree", table.userId.asc().nullsLast().op("text_ops"), table.completedAt.asc().nullsLast().op("text_ops")),
	foreignKey({
			columns: [table.quizId],
			foreignColumns: [quizzes.id],
