    ...
    <section className="relative flex min-h-[calc(100dvh-4rem)] flex-col items-center justify-center overflow-hidden px-4 md:px-8 lg:px-12">
      ...
      <div className="z-10 flex w-full max-w-5xl flex-col items-center text-center">
        ...
        <div className="relative">
          <h1 className="text-5xl font-extrabold tracking-tight select-none sm:text-6xl md:text-7xl lg:text-8xl">
      ...
      <motion.button
        ...
        onClick={() => {
          const steps =
            document.querySelectorAll<HTMLElement>('[data-home-step]');
          if (steps[1]) steps[1].scrollIntoView({ behavior: 'smooth' });
        }}
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 1, duration: 1 }}
        className="absolute bottom-8 left-1/2 flex -translate-x-1/2 cursor-pointer flex-col items-center gap-2 text-gray-400 transition-colors hover:text-(--accent-primary) dark:text-white/50 dark:hover:text-(--accent-primary)"
