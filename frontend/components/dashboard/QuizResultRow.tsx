...
export function QuizResultRow({ attempt, locale }: QuizResultRowProps) {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 sm:grid sm:grid-cols-[auto_minmax(0,2fr)_1fr_auto_20px] sm:gap-3 md:hidden">
        {catStyle && (
          <Image
            src={catStyle.icon}
            alt=""
            width={32}
            height={32}
            className={`shrink-0 sm:h-5 sm:w-5 ${catStyle.iconClassName ?? ''}`}
          />
        )}
        <div className="ml-1 min-w-0 flex-1 space-y-1 sm:space-y-1.5">
          <div className="truncate text-sm leading-tight font-medium text-gray-900 md:text-base dark:text-white">
        ...
        {!isMastered ? (
          <ChevronRight className="-ml-1 h-4 w-4 shrink-0 text-gray-300 transition-transform duration-300 group-hover:translate-x-0.5 group-hover:text-(--accent-primary) dark:text-gray-600" />
        ) : (
      ...
      <div className="hidden items-center gap-2 md:grid md:grid-cols-[minmax(0,4fr)_1fr_1.5fr_1fr_1fr_1fr_20px] lg:grid-cols-[minmax(0,4fr)_1fr_1.5fr_1fr_1fr_1fr_1.2fr_20px]">
        ...
        {/* Date — visible on lg+ */}
        <div
          className="hidden text-center text-xs text-gray-400 lg:block"
