import React from 'react';
  ...
  // Types matching the explanation JSON structure
  interface TextNode {
    text: string;
    bold?: boolean;
    italic?: boolean;
  ...
  // Render list item children (recursive)
  ...
  export default function ExplanationRenderer({ blocks }: ExplanationRendererProps) {
    return (
            ...
            case 'paragraph':
              return (
                <p key={blockIndex} className="leading-relaxed">
                  {block.children.map((child, i) => renderText(child as TextNode,
  i))}
                </p>
              );
            ...
            case 'code':
              return (
                <pre
                  key={blockIndex}
                  className="bg-gray-900 text-gray-100 p-6 rounded-lg
  overflow-x-auto"
                >
                  <code className="text-sm font-mono block leading-relaxed">
                    {block.children.map((child) => (child as
  TextNode).text).join('')}
                  </code>
                </pre>
              );
            ...
            case 'numberedList':
              return (
                <ol key={blockIndex} className="list-decimal list-inside space-y-2
  ml-4">
            ...
            case 'bulletList':
              return (
                <ul key={blockIndex} className="list-disc list-inside space-y-2
  ml-4">
                  {renderChildren(block.children)}
                </ul>
              );
