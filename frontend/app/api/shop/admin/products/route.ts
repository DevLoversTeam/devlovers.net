  ...
  try {
    ...
    if (!(imageFile instanceof File) || imageFile.size === 0) {
      return NextResponse.json(
        { error: "Image file is required", code: "IMAGE_REQUIRED", field: "image" },
        { status: 400 },
      )
    }
    ...
    try {
      const inserted = await createProduct({ ...parsed.data, image: imageFile })
      return NextResponse.json(
        {
          success: true,
          product: inserted,
      ...
      if (error instanceof Error && error.message === "Failed to upload image to Cloudinary") {
        return NextResponse.json(
          { error: "Failed to upload product image", code: "IMAGE_UPLOAD_FAILED", field: "image" },
          { status: 502 },
        )
      }
