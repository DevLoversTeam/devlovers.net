...
import { approveReturnRequest } from '@/lib/services/shop/returns';
...
export async function POST(
  ...
  try {
    ...
    const csrfRes = requireAdminCsrf(request, 'admin:returns:approve');
    if (csrfRes) {
    ...
    if (error instanceof InvalidPayloadError) {
      logWarn('admin_return_approve_rejected', {
        ...baseMeta,
        returnRequestId: returnRequestIdForLog,
        code: error.code,
      });
    ...
    logError('admin_return_approve_failed', error, {
      ...baseMeta,
      returnRequestId: returnRequestIdForLog,
      code: 'ADMIN_RETURN_APPROVE_FAILED',
    });
