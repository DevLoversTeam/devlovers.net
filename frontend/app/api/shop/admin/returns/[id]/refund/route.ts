...
import { refundReturnRequest } from '@/lib/services/shop/returns';
...
export async function POST(
  ...
  try {
    ...
    const csrfRes = requireAdminCsrf(request, 'admin:returns:refund');
    if (csrfRes) {
    ...
    if (error instanceof InvalidPayloadError) {
      logWarn('admin_return_refund_rejected', {
        ...baseMeta,
        returnRequestId: returnRequestIdForLog,
        code: error.code,
      });
    ...
    logError('admin_return_refund_failed', error, {
      ...baseMeta,
      returnRequestId: returnRequestIdForLog,
      code: 'ADMIN_RETURN_REFUND_FAILED',
    });
