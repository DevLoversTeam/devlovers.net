...
} from '@/lib/services/shop/shipping/admin-edit';
import {
  sanitizeShippingErrorForLog,
  sanitizeShippingLogMeta,
} from '@/lib/services/shop/shipping/log-sanitizer';
import { orderIdParamSchema } from '@/lib/validation/shop';
import { adminOrderShippingEditSchema } from '@/lib/validation/shop-admin-shipping';
...
export async function PATCH(
  ...
  const blocked = guardBrowserSameOrigin(request);
  if (blocked) {
    logWarn('admin_orders_shipping_edit_origin_blocked', {
      ...baseMeta,
      code: 'ORIGIN_BLOCKED',
      durationMs: Date.now() - startedAtMs,
    });
  ...
  try {
    const adminUser = await requireAdminApi(request);
    const csrfRes = requireAdminCsrf(request, 'admin:orders:shipping:edit');
    if (csrfRes) {
      logWarn('admin_orders_shipping_edit_csrf_rejected', {
        ...baseMeta,
        code: 'CSRF_REJECTED',
        durationMs: Date.now() - startedAtMs,
      });
    ...
    const parsedPayload = adminOrderShippingEditSchema.safeParse(rawBody);
    if (!parsedPayload.success) {
      return noStoreJson(
        {
          code: 'INVALID_PAYLOAD',
    ...
    if (error instanceof AdminOrderShippingEditError) {
      logWarn('admin_orders_shipping_edit_rejected', {
        ...baseMeta,
        orderId: orderIdForLog,
        code: error.code,
        durationMs: Date.now() - startedAtMs,
    ...
    logError(
      'admin_orders_shipping_edit_failed',
      sanitizeShippingErrorForLog(error, 'Admin shipping edit failed.'),
      sanitizeShippingLogMeta({
        ...baseMeta,
        orderId: orderIdForLog,
        code: 'ADMIN_SHIPPING_EDIT_FAILED',
        durationMs: Date.now() - startedAtMs,
      })
    );
